...
def handle_fault(
    ...
    existing = models.find_recent(machine, fault_code) if machine else None
    if existing:
        return existing
    ...
    wo = build_work_order(diag, translate=translate, fault_arrived_at=fault_arrived_at, source=source)
